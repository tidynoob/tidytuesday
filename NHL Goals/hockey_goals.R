...
career_goals <- game_goals %>% 
  ...
  ungroup() %>% 
  complete(nesting(player), year = seq(min_year, max_year, 1L)) %>% # add full length of years for each player
  left_join(career_years, by = c("player")) %>%
  group_by(player) %>% 
  mutate(career_goals = case_when(year < year_began ~ 0,
